...
""")
	...
	WHERE pv.salePrice IS NOT NULL AND pv.salePrice <> 0 AND pv.salePrice < pv.price
...
""")
	...
	GROUP BY p.id, p.name, p.slug, p.thumbnail
""")
    Page<ProductSaleResponse> findAllByCategoryId(@Param("categoryId") int categoryId, Pageable pageable);
