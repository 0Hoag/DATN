...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT u FROM User u JOIN u.roles r WHERE u.deletedAt IS NULL AND " + "LOWER(r.name) = LOWER(:roleName)")
    ...
    @Query("SELECT u FROM User u JOIN u.roles r WHERE u.deletedAt IS NULL AND "
            ...
            + "OR u.phone LIKE CONCAT('%', :keyword, '%')"
            + ")")
    Page<User> findByKeywordAndRoleName(
            @Param("keyword") String keyword, @Param("roleName") String roleName, Pageable pageable);
