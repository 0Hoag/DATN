...
public interface DashboardRepository extends JpaRepository<Order, Integer> {
    ...
    @Query(
            "SELECT new com.fpl.datn.dto.response.ChartDataPoint(FUNCTION('DATE_FORMAT', o.createdAt, '%Y-%m'), SUM(o.totalAmount)) "
    ...
    @Query(
            "SELECT new com.fpl.datn.dto.response.ChartDataPoint(FUNCTION('DATE_FORMAT', o.createdAt, '%Y-%m'), COUNT(o)) "
    ...
    @Query(
            "SELECT new com.fpl.datn.dto.response.ChartDataPoint(FUNCTION('DATE_FORMAT', od.order.createdAt, '%Y-%m'), SUM(od.quantity)) "
