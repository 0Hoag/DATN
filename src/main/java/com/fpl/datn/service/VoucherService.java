...
// Cập nhật trạng thái voucher hết hạn trước khi search
    ...
    public PageResponse<VoucherResponse> getAvailableForUser(int page, int size) {
        ...
        var pageData = repository.findByIsActiveTrueAndStartAtBeforeAndEndAtAfter(now, now, pageable);
        var data = pageData.stream().map(mapper::toVoucherResponse).toList();
    ...
    public void saveVoucherForUser(int voucherId, int userId) {
        var voucher = repository.findById(voucherId).orElseThrow(() -> new AppException(ErrorCode.VOUCHER_NOT_FOUND));

        boolean alreadySaved = userVoucherRepository.existsByUser_IdAndVoucher_Code(userId, voucher.getCode());
        if (alreadySaved) {
            throw new AppException(ErrorCode.VOUCHER_OVERUSED);
        }

        var userVoucher = ZUserVoucher.builder()
    ...
    public PageResponse<VoucherResponse> getSavedVouchers(int userId, int page, int size) {
        Pageable pageable = PageRequest.of(page - 1, size);
        var pageData = userVoucherRepository.findByUser_Id(userId, pageable);
        var data = pageData.stream()
                .map(zuv -> mapper.toVoucherResponse(zuv.getVoucher()))
                .toList();
