...
public class CategoryService {
    ...
    public Boolean createCategory(CategoryRequest request) {
        ...
        if (request.getParent() != null && !request.getParent().isEmpty()) {
            Category parent = repo.findAll().stream()
                    .filter(c -> c.getName().equals(request.getParent()))
                    .findFirst()
    ...
    public CategoryResponse detail(Integer id) {
        Category category = repo.findById(id).orElseThrow(() -> new AppException(ErrorCode.CATEGORIES_NOT_EXISTED));
