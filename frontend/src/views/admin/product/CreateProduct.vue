        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="form-check mb-3">
              <label for="showHome" class="form-label fw-bold"
      ...
      <div class="col-9 mt-3">
        <div class="card">
          <div class="card-body">
            ...
            <!-- Tạo biến thể -->
            <a-button type="primary" class="my-3" @click="generateVariants"
              ...
              <a-collapse-panel v-for="(variant, index) in variants" :key="index">
                <template #header>
                  <div class="d-flex justify-content-between">
                    ...
                    <a-button type="primary" danger @click="removeVariant(variant)"
                      >Xóa</a-button
                    >
                  </div>
                </template>
                <div class="row g-3">
                  <a href="javascript:void(0)" @click="openMediaModal(index)"
                    >Upload ảnh</a
                  >
                  <div class="col-md-12">
                    <p>Ảnh đại diện</p>
...
// Hàm tạo tên biến thể dựa trên tên sản phẩm và các thuộc tính
function generateVariantName(productName, attributes) {
  // attributes: [{attr, value, id}]
  if (!productName) return attributes.map((a) => a.value).join(" - ");
  return productName + " " + attributes.map((a) => a.value).join("/");
}
...
const generateVariants = () => {
  ...
  variants.value = combinations.map((comb) => {
    ...
    return {
      ...
      price: 0,
      salePrice: 0,
      variantName: generateVariantName(productModel.value.name, comb),
      quantity: 0,
      sold: 0,
      isActive: true,
