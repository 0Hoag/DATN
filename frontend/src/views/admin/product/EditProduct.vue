    ...
    <template #body>
      <a-tabs v-model:activeKey="activeKey">
        ...
        <a-tab-pane key="2" tab="Thư viện">
          ...
          <div class="my-3">
            <button class="btn btn-primary" @click="isGlobalThumbnail ? confirmImageGlobalThumbnail() : confirmImageSelection()">
              Chọn {{ selectedIndexes.length > 0 ? selectedIndexes.length + " ảnh" : "" }}
            </button>
            <button class="btn btn-secondary mx-2" v-if="selectedIndexes.length > 0" @click="clearImageSelection">Hủy chọn tất cả</button>
          </div>
        </a-tab-pane>
      </a-tabs>
    </template>
...
<script setup>
  ...
  function generateVariantTitles() {
    ...
    productModel.value.productVariants.forEach((variant) => {
      const title = variant.attributeValues.map((attr) => `${attr.attributeName.toUpperCase()} ${attr.attributeValue}`).join(" - ");
      variant.title = title;
    });
  }
